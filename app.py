...
# Setup logging (Improved format slightly)
logging.basicConfig(
    filename=LOG_FILE,
    level=logging.INFO,
...
def init_db():
    try:
        ...
        raise # Critical error, stop the app
...
def save_user_data(email, website):
    ...
    try:
        ...
        return True
         ...
         # Decide if this should be True (logged) or False (potential error to user)
         return True # Assume logging is sufficient
    except sqlite3.Error as e:
...
def fetch_and_extract_texts_parallel(urls):
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (compatible; KeywordCheckerBot/1.1; +http://example.com/bot)'} # Replace with actual bot info URL if available
    ...
    with requests.Session() as session:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS_FETCH, thread_name_prefix='fetch_url') as executor:
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    # result() will re-raise exceptions from extract_text_from_url
                    res_url, text = future.result()
                    if text: # Only add if text extraction was successful
                        url_texts[res_url] = text
                except Exception as e:
                    # Log errors from the future execution itself (less common)
                    logging.error(f"Future execution failed for {url}: {e}", exc_info=True)
    ...
    try:
        with session.get(normalized_base_url, timeout=10) as response:
            ...
            for a_tag in soup.select('a[href]'):
                href = a_tag.get('href')
                if not href or href.startswith(('mailto:', 'tel:', 'javascript:')): # Skip non-web links
                    continue
        ...
        logging.error(f"Failed to get base URL {normalized_base_url}: {e}", exc_info=False) # Log less verbosely for request errors
        return None
    except Exception as e:
...
def get_dynamic_stop_words(text, lang, top_n=50):
    ...
    # {3,} ensures words are at least 3 chars long
    ...
    general_stop_words = {
        'en': {
            ...
            'http', 'https', 'www', 'com', 'org', 'net', 'gov', 'edu', 'html', 'pdf', 'inc', 'corp', 'ltd'
        },
        'fa_ar': {
            ...
            'حتی', 'فقط', 'باید', 'نباید', 'باشد', 'نباشد', 'آیا', 'کدام', 'چه', 'چگونه', 'کجا', 'کی', 'چیست',
            # Prepositions & Conjunctions
            'ضمن', 'بین', 'روی', 'زیر', 'بالای', 'کنار', 'مقابل', 'طبق', 'مانند', 'مثل', 'حدود', 'طی', 'طریق',
            # Arabic common words often mixed in Persian text
            'علیه', 'الا', 'فی', 'کل', 'غیر', 'ذلک', 'من', 'الی', 'عن', 'حتیٰ', 'اذا', 'لم', 'لن', 'کان', 'لیس', 'قد', 'ما',
            # Common suffixes/prefixes often found as separate words by regex
             'ها', 'تر', 'ترین', 'می', 'نمی', 'بی',
            # Common website/context words
            'میلادی', 'شمسی', 'هجری', 'سال', 'ماه', 'روز', 'شنبه', 'یکشنبه', 'دوشنبه', 'سهشنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه',
            'شرکت', 'سازمان', 'اداره', 'موسسه', 'دانشگاه', 'گروه', 'بخش', 'واحد',
    ...
    # logging.debug(f"Dynamic-only stop words: {dynamic_only}")
    ...
    for url, text in url_texts_dict.items():
        ...
        try:
            ...
            if index != -1:
                ...
                # Clean up preview text slightly
                ...
                if score > best_match["score"]:
                    ...
                    # Optimization: If only first match is needed, uncomment break
                    # break
        except Exception as e:
            # Log errors during the search process itself (rare)
    ...
    for folder in folders_to_clean:
        try:
            ...
            if not os.path.isdir(folder):
                logging.warning(f"Cleanup folder does not exist: {folder}")
                continue
...
def index():
    ...
    error_message = None
    # Get form data regardless of method for template rendering
    ...
    if request.method == 'POST':
        ...
        if validation_errors:
            ...
            # Return early, passing back submitted values for form persistence
            return render_template("index.html", error=error_message, website=original_website_url, email=email)
        # --- End Validation ---
        ...
        if not save_user_data(email, website_url):
            ...
            # flash("Notice: Could not save user statistics.", "warning") # Requires session setup
        ...
        try:
            ...
            try:
                ...
                # Use .iloc[:, 0] to get first column, dropna, ensure string, get unique
                keywords = df.iloc[:, 0].dropna().astype(str).unique().tolist()
                if not keywords:
                    raise ValueError("هیچ کلمه کلیدی معتبری در ستون اول یافت نشد.")
            ...
            with requests.Session() as session:
                # No need to update headers here, fetch_and_extract_texts_parallel does it
                urls_to_check = get_internal_urls(website_url, session)

            if urls_to_check is None: # Critical error during base URL fetch
                raise ConnectionError(f"امکان دسترسی به وب‌سایت اصلی ({website_url}) یا دریافت لینک‌ها وجود ندارد.")
            if not urls_to_check: # Base URL ok, but no links found (or only base URL)
                 logging.warning(f"No internal URLs found beyond the base for {website_url}")
                 # Decide if this is an error or just continue with base URL
            ...
            if not url_texts:
                # This can happen if all URLs failed or had non-HTML content
            ...
            # Using a sample might be faster if there's a *lot* of text, but joining all is more accurate
            ...
            # logging.info(f"Total phrases/words to check (incl. individuals): {len(phrases_to_check)}")
            # *** End Optional Section ***

            if not phrases_to_check:
                 raise ValueError("پس از پردازش، هیچ کلمه کلیدی معتبری برای بررسی وجود ندارد.")
            logging.info(f"Total unique phrases to check: {len(phrases_to_check)}")


            # --- Check Keywords Concurrently ---
            check_start_time = time.time()
            temp_results = []
            # Use thread name prefix for easier debugging
            with ThreadPoolExecutor(max_workers=MAX_WORKERS_CHECK, thread_name_prefix='check_kw') as executor:
                ...
                for future in as_completed(future_to_phrase):
                    try:
                        ...
                        # Append an error entry for this keyword
                        temp_results.append({"keyword": phrase, "found": False, "url": "Error", "score": 0, "preview": f"Processing Error: {e}"})
            ...
            if results_data:
                ...
                # This case should be rare if phrases_to_check wasn't empty
                error_message = "پردازش کامل شد، اما هیچ نتیجه‌ای برای کلمات کلیدی یافت نشد."
            ...
            error_message = f"یک خطای پیش‌بینی نشده رخ داد: {e}"
        finally:
    ...
    return render_template("index.html",
                           ...
                           error=error_message,
                           website=website_url, # Use potentially corrected URL
                           email=email)
...
def download(filename):
    ...
    if not safe_filename or safe_filename != filename: # Check if filename was altered
...
if __name__ == '__main__':
    init_db()
    # Run cleanup in a separate thread or scheduled task in production
    # For development, running on startup is okay.
    cleanup_old_files()
    # Use debug=False and run with a proper WSGI server (like Gunicorn) in production
    # Example: gunicorn --workers 4 --bind 0.0.0.0:5000 your_app_module:app
